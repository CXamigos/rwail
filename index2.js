  ...
  const arras = (function () {
    ...
    const loadScript = function () {
      ...
      realFetch("https://arras.io")
        ...
        .catch((err) => {
          log(
            "FATAL: Could not fetch from arras.io. Please check network or use a valid cache file.",
            err,
          );
        });
    };
    ...
    const run = function (x, config, oa) {
        ...
        const simulatedContext2D = {
          ...
          fillText: function () {
            ...
            if (
              ...
              if (
                !a[0].startsWith(`You're using an ad blocker.`) &&
                a[0] !== "Respawn" &&
                a[0] !== "Back" &&
                a[0] !== "Reconnect" &&
                a[0].length > 2
                ...
                if (
                  a[0].startsWith("You have been killed by ") ||
                  a[0] === "You have died a stupid death."
                ) {
                  died = true;
                }
              }
            ...
            if (
              this.font === "bold 7.5px Ubuntu" &&
              this.fillStyle === "rgb(231,137,109)"
            ) {
              if (
                a[0] === "You have been temporarily banned from the game." ||
                a[0] ===
                  "Your IP address have been blacklisted due to suspicious activities."
              ) {
                disconnected = true;
                destroy();
                log("[arras]", a[0]);
                ...
                if (!destroyed) {
                  ...
                  if (connected) {
                    if (reconnectCount < config.reconnectAttempts) {
                      reconnectCount++;
                      log(
                        `Attempting to reconnect in ${config.reconnectDelay / 1000}s... (${reconnectCount}/${config.reconnectAttempts})`,
                      );
                      ...
                      log(
                        `Max reconnection attempts reached (${config.reconnectAttempts}). Will not reconnect.`,
                      );
                    }
                  }
                }
        ...
        log: new Proxy(global.console.log, {
          apply: function (a, b, args) {
            if (
              ...
              (args[0] &&
                args[0].startsWith &&
                args[0].startsWith("%cHackers have been known"))
            ) {
              return;
            }
      ...
      async function onJoin() {
        ...
        let waited;
        let tankPath = tanks[target.tank] ? tanks[target.tank].path : "";
        for (const key of tankPath) {
          if (key === "wait") {
            await waitTime(100);
        ...
        if (target.feed) {
          ...
          let tankBuildStr = tanks[target.tank]
            ? tanks[target.tank].build || builds.basic
            : builds.basic;
          build = tankBuildStr.split("/");
        }
      ...
      const mainInterval = setInterval(function () {
        ...
        if (a) {
          ...
          if (hasJoined) {
            ...
            // Forward-Backward-Forward dance after joining
            const dance = async () => {
              isDancing = true;
              const press = (key, duration) =>
                new Promise((res) => {
                  trigger.keydown(key);
                  setTimeout(() => {
                    trigger.keyup(key);
              ...
              await press("KeyS", 100);
              await press("KeyW", 100);

              isDancing = false;

              if (
                tanks instanceof Array &&
                tanks[target.tank].path.find((key) => key instanceof Array)
              ) {
          ...
          if (inGame) {
            // Chat logic: handle broadcast from controller.user.js
            if (target.chatMessage) {
              const msg = target.chatMessage;
              if (target.chatSpam) {
                if (i % 10 === 0) {
                  // Every ~1 second
                  controller.chat(msg);
                ...
                controller.chat(msg);
                target.chatMessage = ""; // Clear after one-shot
              }
            } else if (i % 10 === 0) {
              // Default spam if no specific message set
              controller.chat("free farm: discord.gg/fQFTCMC5hY");
            }
            ...
            controller[
              target.mouseDown && !target.feed ? "mouseDown" : "mouseUp"
            ]();
            controller[
              target.rMouseDown && !target.feed ? "mouseDown" : "mouseUp"
            ](2);
          ...
          if (died) {
            ...
            let interval = setInterval(function () {
                ...
                for (let i = 0; i < 30; i++) {
                  let r = 100 + 900 * Math.random(),
                    q = 100 + 900 * Math.random(),
                    p = 0.5 + Math.random();
                ...
                if (config.autoRespawn) {
                  ...
                  const interv = setInterval(() => {
                    ...
                    // Re-apply held keys after respawn
                    for (const key of config.keysHold) {
                      trigger.keydown("Key" + key.toUpperCase());
                    }
      ...
      global.WebSocket = global.window.WebSocket = new Proxy(WebSocket, {
        construct: function (a, b, c) {
          ...
          d.addEventListener("message", function (e) {
            let u = Array.from(new Uint8Array(e.data));
          });
    ...
    let arras = {
      ...
      create: function (o) {
        if (!ready) {
          log(
            "Warning: 'create' called before arras was ready. It will be queued.",
          );
        }
  ...
  onMessage((message) => {
    if (message.type === "start") {
      ...
      if (config.targetX !== undefined) {
        target.x = config.targetX;
      }
      if (config.targetY !== undefined) {
        target.y = config.targetY;
      }
      if (config.tank !== undefined) {
        target.tank = config.tank;
      }
      if (config.followMouse !== undefined) {
        target.followMouse = config.followMouse;
      }
      target.chatMessage = config.chatMessage || "";
      target.chatSpam = config.chatSpam !== undefined ? config.chatSpam : false;
      target.autoFire = config.autoFire !== undefined ? config.autoFire : false;
      ...
      if (currentBotInterface.log) {
        currentBotInterface.log(
          `Bot state is now: ${isPaused ? "PAUSED" : "RESUMED"}`,
        );
      }
      ...
      if (message.autoFire !== undefined) {
        target.autoFire = message.autoFire;
      }
    } else if (message.type === "destroy") {
      if (currentBotInterface && currentBotInterface.destroy) {
        currentBotInterface.destroy();
      }
      process.exit(0);
    } else if (message.type == "tankselect") {
      target.tank = message.tank;
      if (inGame) {
        onJoin(); // Re-run join logic to apply new tank and build
      }
    } else if (message.type == "chat") {
      target.chatMessage = message.message;
      target.chatSpam = message.spam;
    }
  });
