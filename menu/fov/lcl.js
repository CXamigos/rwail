const express = require('express');
const http = require('http');
const https = require('https');
const net = require('net');
const path = require('path');
const tls = require('tls');
const { WebSocketServer, WebSocket: ServerWebSocket } = require('ws');

const app = express();
const port = 3000;

const INITIAL_SERVER_PACKET = Buffer.from('00010001ffa47515f98112ce', 'hex');
const SERVER_PROTOCOLS = ['arras.io#v1.4+sls+et0', 'arras.io'];
const HANDSHAKE_B = Buffer.from(INITIAL_SERVER_PACKET.subarray(4)).reverse().toString('hex');

function normalizeCloseCode(code) {
  if (typeof code !== 'number' || !Number.isInteger(code)) return undefined;
  if (code === 1000) return 1000;
  if (code >= 1001 && code <= 1014 && ![1004, 1005, 1006].includes(code)) return code;
  if (code >= 3000 && code <= 4999) return code;
  return undefined;
}

function normalizeCloseReason(reason) {
  if (reason == null) return '';
  const text = Buffer.isBuffer(reason) ? reason.toString() : String(reason);
  return Buffer.byteLength(text) > 123 ? text.slice(0, 123) : text;
}

app.use('/wasm', express.static(path.join(__dirname, 'wasm')));

app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'simulation.html'));
});

const server = http.createServer(app);

const proxyWss = new WebSocketServer({
  noServer: true,
  handleProtocols(protocols) {
    if (protocols.has('arras.io#v1.4+sls+et0')) return 'arras.io#v1.4+sls+et0';
    if (protocols.has('arras.io')) return 'arras.io';
    return false;
  },
});

function normalizeTargetUrl(rawTarget) {
  let targetUrl = rawTarget;
  try {
    targetUrl = decodeURIComponent(rawTarget);
  } catch (_) {}
  const parsed = new URL(targetUrl);
  parsed.searchParams.set('b', HANDSHAKE_B);
  parsed.searchParams.set('t', Math.floor(Date.now() / 1000).toString());
  targetUrl = parsed.toString();
  return targetUrl;
}

function normalizeProxyUrl(rawProxy) {
  if (!rawProxy) return '';
  try {
    return decodeURIComponent(rawProxy);
  } catch (_) {
    return String(rawProxy);
  }
}

function buildProxyAuthorizationHeader(proxyUrl) {
  if (!proxyUrl.username && !proxyUrl.password) return '';
  let username = decodeURIComponent(proxyUrl.username || '');
  let password = decodeURIComponent(proxyUrl.password || '');
  return `Basic ${Buffer.from(`${username}:${password}`).toString('base64')}`;
}

class ProxyTunnelAgent extends https.Agent {
  constructor(proxyAddress) {
    super({
      keepAlive: true
    });
    this.proxyUrl = new URL(proxyAddress);
    this.proxyAuthorization = buildProxyAuthorizationHeader(this.proxyUrl);
  }

  createConnection(options, callback) {
    let proxyPort = Number(this.proxyUrl.port || (this.proxyUrl.protocol === 'https:' ? 443 : 80));
    let connectOptions = {
      host: this.proxyUrl.hostname,
      port: proxyPort
    };
    let targetHost = options.host || options.hostname;
    let targetPort = Number(options.port || 443);
    let proxySocket = this.proxyUrl.protocol === 'https:'
      ? tls.connect({
          ...connectOptions,
          servername: this.proxyUrl.hostname
        })
      : net.connect(connectOptions);

    let settled = false;
    let responseBuffer = Buffer.alloc(0);
    let onError = error => {
      if (settled) return;
      settled = true;
      callback(error);
    };
    let onConnect = () => {
      let headers = [
        `CONNECT ${targetHost}:${targetPort} HTTP/1.1`,
        `Host: ${targetHost}:${targetPort}`,
        'Proxy-Connection: Keep-Alive',
        'Connection: Keep-Alive'
      ];
      if (this.proxyAuthorization) headers.push(`Proxy-Authorization: ${this.proxyAuthorization}`);
      headers.push('', '');
      proxySocket.write(headers.join('\r\n'));
    };
    let onData = chunk => {
      if (settled) return;
      responseBuffer = Buffer.concat([responseBuffer, chunk]);
      let headerEnd = responseBuffer.indexOf('\r\n\r\n');
      if (headerEnd === -1) return;

      let headerText = responseBuffer.subarray(0, headerEnd).toString('latin1');
      let statusLine = headerText.split('\r\n')[0] || '';
      if (!/^HTTP\/1\.[01] 200\b/.test(statusLine)) {
        onError(new Error(`Proxy CONNECT failed: ${statusLine || 'Unknown response'}`));
        proxySocket.destroy();
        return;
      }

      settled = true;
      proxySocket.off('data', onData);
      proxySocket.off('error', onError);

      let secureSocket = tls.connect({
        socket: proxySocket,
        servername: net.isIP(targetHost) ? '' : targetHost
      });
      secureSocket.once('error', callback);
      secureSocket.once('secureConnect', () => {
        secureSocket.off('error', callback);
        callback(null, secureSocket);
      });
    };

    proxySocket.once(this.proxyUrl.protocol === 'https:' ? 'secureConnect' : 'connect', onConnect);
    proxySocket.on('data', onData);
    proxySocket.on('error', onError);
    return undefined;
  }
}

function createUpstreamSocket(targetUrl, protocols, headers, upstreamProxy) {
  let options = {
    headers,
    perMessageDeflate: true
  };
  if (upstreamProxy) options.agent = new ProxyTunnelAgent(upstreamProxy);
  return new ServerWebSocket(targetUrl, protocols, options);
}

proxyWss.on('connection', (client, req, targetUrl, upstreamProxy) => {
  const clientUserAgent = req.headers['user-agent'] || 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/145.0.0.0 Safari/537.36';
  const clientLanguage = req.headers['accept-language'] || 'en-US,en;q=0.9';
  console.log(`Proxying game socket to ${targetUrl}${upstreamProxy ? ` via ${new URL(upstreamProxy).hostname}` : ''}`);
  const upstreamHeaders = {
    'User-Agent': clientUserAgent,
    'Origin': 'https://arras.io',
    'Pragma': 'no-cache',
    'Cache-Control': 'no-cache',
    'Accept-Encoding': 'gzip, deflate, br, zstd',
    'Accept-Language': clientLanguage,
  };
  const upstream = createUpstreamSocket(targetUrl, SERVER_PROTOCOLS, upstreamHeaders, upstreamProxy);

  let upstreamOpen = false;
  let firstServerPacketSent = false;
  const pendingPackets = [];

  function isUpstreamAlive() {
    return upstream.readyState === ServerWebSocket.CONNECTING || upstream.readyState === ServerWebSocket.OPEN;
  }

  function flushPendingPackets() {
    let isOpen = upstream.readyState === ServerWebSocket.OPEN;
    while (pendingPackets.length && isOpen) {
      const packet = pendingPackets.shift();
      if (!firstServerPacketSent) {
        firstServerPacketSent = true;
        upstream.send(INITIAL_SERVER_PACKET);
      } else {
        upstream.send(packet);
      }
      isOpen = upstream.readyState === ServerWebSocket.OPEN;
    }
  }

  client.on('message', (packet, isBinary) => {
    const normalizedPacket = isBinary ? packet : Buffer.from(packet);
    pendingPackets.push(normalizedPacket);
    if (upstreamOpen) flushPendingPackets();
  });

  client.on('close', (code, reason) => {
    if (isUpstreamAlive()) {
      const safeCode = normalizeCloseCode(code);
      const safeReason = normalizeCloseReason(reason);
      if (safeCode === undefined) upstream.close();
      else upstream.close(safeCode, safeReason);
    }
  });

  client.on('error', () => {
    if (isUpstreamAlive()) {
      upstream.close();
    }
  });

  upstream.on('open', () => {
    upstreamOpen = true;
    console.log('Upstream game socket opened');
    flushPendingPackets();
  });

  upstream.on('message', (packet, isBinary) => {
    if (client.readyState === ServerWebSocket.OPEN) {
      client.send(packet, { binary: isBinary });
    }
  });

  upstream.on('close', (code, reason) => {
    const safeCode = normalizeCloseCode(code);
    const closeReason = normalizeCloseReason(reason);
    console.log(`Upstream game socket closed with code ${code}${closeReason.length ? `: ${closeReason}` : ''}`);
    if (client.readyState === ServerWebSocket.OPEN || client.readyState === ServerWebSocket.CONNECTING) {
      if (safeCode === undefined) client.close();
      else client.close(safeCode, closeReason);
    }
  });

  upstream.on('error', (error) => {
    console.log(`Upstream game socket error: ${error.message}`);
    if (client.readyState === ServerWebSocket.OPEN || client.readyState === ServerWebSocket.CONNECTING) {
      client.close(1011, 'Upstream WebSocket error');
    }
  });
});

server.on('upgrade', (req, socket, head) => {
  const requestUrl = new URL(req.url, `http://${req.headers.host}`);
  if (requestUrl.pathname !== '/game-proxy') {
    socket.destroy();
    return;
  }

  const rawTarget = requestUrl.searchParams.get('target');
  if (!rawTarget) {
    socket.destroy();
    return;
  }

  const targetUrl = normalizeTargetUrl(rawTarget);
  const upstreamProxy = normalizeProxyUrl(requestUrl.searchParams.get('proxy'));
  if (!/^wss?:\/\//i.test(targetUrl)) {
    socket.destroy();
    return;
  }

  proxyWss.handleUpgrade(req, socket, head, (ws) => {
    proxyWss.emit('connection', ws, req, targetUrl, upstreamProxy);
  });
});

server.listen(port, () => {
  console.log(`Server listening at http://localhost:${port}`);
  console.log(`Game proxy listening at ws://localhost:${port}/game-proxy`);
});
