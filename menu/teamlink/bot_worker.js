...
if (Array.isArray(workerData)) {
    [user_agent, proxy, server_code, upgrade_data, bot_name, bot_pathfind, followbot_config, writer, options] = workerData;
} else if (workerData && typeof workerData === "object") {
    ({ user_agent, proxy, server_code, upgrade_data, bot_name, bot_pathfind, followbot_config, writer, options } = workerData);
}

options = options || {};
const teamlink_mode = options.mode === "teamlink";
            ...
            m = [
                [(e, r, n) => {
                                    ...
                                    a.addEventListener("close", t => {
                                        ...
                                        if (client_opened) {
                                            if (!stop_requested && rejoin && connection_count < 3) {
                                                ...
                                                if (teamlink_mode) {
                                                    parentPort.postMessage({ type: "error", message: `There was an error connecting to #${url}.` });
                                                } else {
                        ...
                        t(e, (() => {
                            ...
                            if (!teamlink_mode) {
                                setInterval(() => {
                                    ...
                                    } else if (bot_upgraded && target.x && target.y) {
                            ...
                            if (new_packet_message) {
                                switch (message_packet[0]) {
                                    ...
                                    case 74:
                                        ...
                                        decoded_packet[0] = 74;
                                        parentPort.postMessage([1, new Uint8Array(decoded_packet).buffer]);
                                    ...
                                    break;
                                    case 70: 
                                        packet_queue.push(packet_constructors.construct_spawn_packet(bot_name, ""));
                                        setTimeout(() => { 
                                    ...
                                    case 109:
                                        ...
                                        if (m.includes("you tried to join is full")) {
                                            if (teamlink_mode) {
                                                parentPort.postMessage({ type: "error", message: "A bot tried to join a full clan." });
                                            } else {
                                                parentPort.postMessage([0, `A bot tried to join a full clan.`]);
                                            }
                                        }
                                        break;
                                    case 75:
                    ...
                    (index, slice) => {
                        ...
                        } else if (!teamlink_mode && bot_upgraded && upgrade_data && !upgrade_data.autospin) {
                            let facing = target.pathfinding_facing ? target.pathfinding_facing : target.facing;
                            u8.set(new Uint8Array([67, facing[0], facing[1], current_move]), 0);
                            return 4;
                        };
